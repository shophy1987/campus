...
namespace shophy\campus\common;

use shophy\campus\exception\ParameterException;
...
class Utils { 
    ...
    static public function checkNotEmptyStr($var, $name)
    {
        if (!self::notEmptyStr($var))
            throw new ParameterException($name . " can not be empty string");
    }

    static public function checkIsUInt($var, $name)
    {
        if (!(is_int($var) && $var >= 0))
            throw new ParameterException($name . " need unsigned int");
    }

    static public function checkNotEmptyArray($var, $name)
    {
        if (!is_array($var) || count($var) == 0) {
            throw new ParameterException($name . " can not be empty array");
        }
    }
